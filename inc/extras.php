...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	 ...
	 */
	function areavoices_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'areavoices' ), max( $paged, $page ) );
		}

		return $title;
	}
	add_filter( 'wp_title', 'areavoices_wp_title', 10, 2 );
	 ...
	 */
	function areavoices_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
	add_action( 'wp_head', 'areavoices_render_title' );
endif;
